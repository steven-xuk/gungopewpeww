    ...
    public class AdvancedShipController : Vehicle
    {
        ...
        public override void FixedUpdate()
        {
            ...
            if (stabilizePitch || stabilizeRoll)
            {
                ...
                if (stabilizeRoll)
                {
                    _stabilizationTorque.z = -Mathf.Clamp01(GetRollAngle() / maxStabilizationTorqueAngle) *
                                             rollStabilizationMaxTorque;
                }

                if (stabilizePitch)
                {
                    _stabilizationTorque.x = Mathf.Clamp01(GetPitchAngle() / maxStabilizationTorqueAngle) *
